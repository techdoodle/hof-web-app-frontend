...
export function Leaderboard() {
  ...
  const userRank = leaderboard?.find(
    (item: LeaderboardItemType) => item.id === userData?.id
  )?.rank;
  const userItem = leaderboard?.find(
    (item: LeaderboardItemType) => item.id === userData?.id
  );
  ...
  const showBottomCard =
    userItem && userRank && userRank > 3 && (!isUserVisible || isMerging);
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {showBottomCard && (
        <div
          className={`fixed left-1/2 transform -translate-x-1/2 p-[1px] rounded-2xl transition-all duration-300 ease-in-out cursor-pointer hover:scale-105 ${
            isMerging
              ? "opacity-0 scale-95 bottom-[50vh]"
              : "opacity-100 scale-100 bottom-[125px]"
          }`}
